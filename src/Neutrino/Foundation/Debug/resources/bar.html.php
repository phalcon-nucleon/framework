...
use Phalcon\Logger;
use Neutrino\Debug\Exceptions\Errors;
...
foreach ($php_errors as $error) {
    if ($error instanceof Errors\DeprecatedErrorException) {
        $errors['deprecated'][] = $error;
    } elseif ($error instanceof Errors\StrictErrorException) {
        $errors['strict'][] = $error;
    } elseif ($error instanceof Errors\NoticeErrorException) {
        $errors['notice'][] = $error;
    } elseif ($error instanceof Errors\WarningErrorException) {
        $errors['warning'][] = $error;
    } elseif ($error instanceof Errors\CustomErrorException) {
        $errors['custom'][] = $error;
    } else {
        $logLevel = Neutrino\Debug\Exceptions\Helper::logLevel($error);
    ...
    <div class="nuc-debug-bar-nav-wrapper">
      <ul class="left">
        ...
        <li class="<?= (Neutrino\Foundation\Debug\length($php_errors) > 0 ? 'with-errors' : 'no-errors') ?>">
          <table style="margin: 0;padding: 0;white-space: nowrap;" class="dropup-content bordered">
            <tbody>
            <?php foreach ($errors as $type => $terrors) : ?>
              <?php $nb = count($terrors); ?>
              <?php if ($nb) : ?>
              <tr data-debug-modal-trigger="debug-phperrors-<?= $type ?>"
                  class="debug-modal-trigger" style="cursor:pointer">
                <td><?= ucwords($type) ?></td>
                <td><span class="bag php-error <?= $type ?>"><?= $nb ?></span></td>
              </tr>
              <?php endif; ?>
            <?php endforeach; ?>
            </tbody>
        ...
        <div class="debug-modal" id="debug-phperrors-<?= $type ?>">
          <div class="debug-modal-content">
            <div style="margin: 0;padding: 0;">
              <?php foreach ($terrors as $error) : ?>
                <div class="php-error <?= $type ?>">
                  <span class="type"><?= \Neutrino\Debug\Exceptions\Helper::verboseType($error) ?></span> :
                  <pre class="msg"><?= ($error->getMessage() ?: 'no message') ?></pre>
                  <span class="file" title="View code">in : <?= Neutrino\Foundation\Debug\file_highlight($error->getFile()) ?> &nbsp;(line: <?= $error->getLine() ?>)</span>
                  <div class="phpfilepart">
                      <?= Neutrino\Foundation\Debug\php_file_part_highlight($error->getFile(), $error->getLine()); ?>
                  </div>
                </div>
              <?php endforeach; ?>
            </div>
