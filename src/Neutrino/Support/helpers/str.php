...
{
    foreach (str_charsArray() as $key => $val) {
...
{
    foreach ((array)$needles as $needle) {
        if ((string)$needle === str_substr($haystack, -str_length($needle))) {
            return true;
        }
    }
 ...
 * @return array
 */
function str_parseCallback($callback, $default)
{
...
{
    $string = '';
    while (($len = str_length($string)) < $length) {
        $size = $length - $len;

        $bytes = str_callRandom($size);

        $string .= str_substr(str_replace(['/', '+', '='], '', base64_encode($bytes)), 0, $size);
    }
 ...
 * @return string
 */
function str_replaceFirst($search, $replace, $subject)
{
