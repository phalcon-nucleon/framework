...
class ArrFuncTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFetch()
    {
        ...
        $this->assertEquals(null, Arr::fetch($array, null));
        $this->assertEquals('baz', Arr::fetch($array, null, 'baz'));
        $this->assertEquals('foo', Arr::fetch($array, 0, 'baz'));
        $this->assertEquals('baz', Arr::fetch($array, 3, 'baz'));
        $this->assertEquals('boo', Arr::fetch($array, 'baz'));
        $this->assertEquals(123, Arr::fetch($array, 'foo.bar'));
        $this->assertEquals(null, Arr::fetch($array, 'null'));
        $this->assertEquals('test', Arr::fetch($array, 'null', 'test'));
    }

    public function testRead()
    {
        $array = ['foo', 'bar', 'baz' => 'boo', 'foo.bar' => 123, 'null' => null];
        $this->assertEquals(null, Arr::read($array, null));
        $this->assertEquals('baz', Arr::read($array, null, 'baz'));
        $this->assertEquals('foo', Arr::read($array, 0, 'baz'));
        $this->assertEquals('baz', Arr::read($array, 3, 'baz'));
        $this->assertEquals('boo', Arr::read($array, 'baz'));
        $this->assertEquals(123, Arr::read($array, 'foo.bar'));
        $this->assertEquals(null, Arr::read($array, 'null'));
        $this->assertEquals(null, Arr::read($array, 'null', 'test'));
    }
    ...
    public function testSet()
    {
        ...
        Arr::set($array, 'products.desk.price.euro', 200);
